...
cursor.execute(
    "CREATE TABLE IF NOT EXISTS paste(id TEXT PRIMARY KEY, title TEXT, date DATE, data TEXT);"
)
...
def paste():
    if request.method == "POST":
        id = rpg.Gen(
            (False, False, 6)
        )  # find a way to make sure that there is no collision
        con = sqlite3.connect("paste.db")
        cursor = con.cursor()
        value = request.form["paste"]
        title = request.form["paste-title"]
        date = str(datetime.now()).split(".")[0]
        res = cursor.execute("SELECT id FROM paste;")
        cursor.execute(
            "INSERT INTO paste VALUES (?, ?, ?,?);", (id, title, date, value)
        )
